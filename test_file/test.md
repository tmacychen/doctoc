...
<!--auto generated by doctoc tools-->
<!--doctoc start-->
- [第一章:Go汇编入门 ](#第一章:Go汇编入门 )
   - ["伪汇编"](##"伪汇编")
   - [分解一个简单程序](##分解一个简单程序)
       - [分析`add`](###分析`add`)
       - [分析`main`](###分析`main`)
   - [关于goroutines，stacks和splits](##关于goroutines，stacks和splits)
       - [Stacks（栈）](###Stacks（栈）)
       - [Splits（拆分)](###Splits（拆分))
       - [没有提及一些技巧](###没有提及一些技巧)
   - [总结](##总结)
...
~r2+16(SP)`，这是调用者事先预留存放返回值的地方。再一次，这里的`"".~r2`在语义上没有具体意义。
